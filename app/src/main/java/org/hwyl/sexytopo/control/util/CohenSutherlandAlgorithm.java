...
public class CohenSutherlandAlgorithm {
    ...
    public static boolean whollyOutside(Coord2D p1, Coord2D p2, Coord2D cp1, Coord2D cp2) {
    ...
    public static boolean clips(Coord2D p1, Coord2D p2, Coord2D cp1, Coord2D cp2) {
        ...
        if (p1code == 0 && p2code == 0) r = false;
        else if (p1code != 0 && p2code != 0 && icode != 0) r = false;
        else r = true;
        return r;
    }
    ...
    public static int bitcode(Coord2D p1, Coord2D p2, Coord2D p) {
        return (above(p1, p2, p) | below(p1, p2, p) | left(p1, p2, p) | right(p1, p2, p));
    }

    /* Gives the 2**3 bit (point above line(p1,p2)) */
    public static int above(Coord2D p1, Coord2D p2, Coord2D p) {
        if (p.y < Math.min(p1.y, p2.y)) return 0x8;
        return 0;
    }
