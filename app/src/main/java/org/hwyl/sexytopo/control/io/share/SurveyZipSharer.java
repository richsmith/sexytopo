...
public class SurveyZipSharer implements Shareable {
    ...
    private Map<String, String> buildZipEntries(Context context, Survey survey) throws Exception {
        ...
        entries.put(
                SurveyFile.withExtension(name, SexyTopoConstants.DATA_EXTENSION),
                SurveyJsonTranslater.toText(survey, versionName, versionCode));
        entries.put(
                SurveyFile.withExtension(name, SexyTopoConstants.METADATA_EXTENSION),
                MetadataTranslater.translate(survey, versionName, versionCode));
        entries.put(
                SurveyFile.withExtension(name, SexyTopoConstants.PLAN_SKETCH_EXTENSION),
                SketchJsonTranslater.translate(
                        survey.getPlanSketch(), survey, versionName, versionCode));
        entries.put(
                SurveyFile.withExtension(name, SexyTopoConstants.EXT_ELEVATION_SKETCH_EXTENSION),
                SketchJsonTranslater.translate(
                        survey.getElevationSketch(), survey, versionName, versionCode));
        return entries;
    }

    private File writeZipToCache(Context context, String surveyName, Map<String, String> entries)
            throws IOException {
