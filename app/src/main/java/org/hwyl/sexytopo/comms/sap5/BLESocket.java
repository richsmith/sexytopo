...
class BLESocket extends BluetoothGattCallback {
    ...
    private static final UUID BLUETOOTH_LE_NRF_CHAR_RW2 =
            UUID.fromString(
                    "6e400002-b5a3-f393-e0a9-e50e24dcca9e"); // read on microbit, write on adafruit
    private static final UUID BLUETOOTH_LE_NRF_CHAR_RW3 =
            UUID.fromString("6e400003-b5a3-f393-e0a9-e50e24dcca9e");
    private static final UUID BLUETOOTH_LE_RN4870_SERVICE =
            UUID.fromString("49535343-FE7D-4AE5-8FA9-9FAFD205E455");
    private static final UUID BLUETOOTH_LE_RN4870_RW =
    ...
    void connect(Context context, BLEListener listener, BluetoothDevice device) throws IOException {
        ...
        context.registerReceiver(
                disconnectBroadcastReceiver,
                new IntentFilter(BuildConfig.APPLICATION_ID + ".Disconnect"));
            ...
            switch (action) {
                case BluetoothDevice.ACTION_PAIRING_REQUEST:
                    ...
                    onSerialConnectError(
                            new IOException(context.getString(R.string.device_pairing_request)));
                    // pairing dialog brings app to background (onPause), but it is still partly
                    // visible (no onStop), so there is no automatic disconnect()
                    break;
    ...
    public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
        ...
        if (newState == BluetoothProfile.STATE_CONNECTED) {
            ...
            if (connected)
                ...
                connected = false;
            else onSerialConnectError(new IOException("gatt status " + status));
        } else {
    ...
    public void onServicesDiscovered(BluetoothGatt gatt, int status) {
        ...
        for (BluetoothGattService gattService : gatt.getServices()) {
            ...
            if (gattService.getUuid().equals(BLUETOOTH_LE_NRF_SERVICE)) {
                ...
                // gattService.getCharacteristics())
                //    Log.d(TAG, "characteristic "+characteristic.getUuid());
                BluetoothGattCharacteristic rw2 =
                        gattService.getCharacteristic(BLUETOOTH_LE_NRF_CHAR_RW2);
                BluetoothGattCharacteristic rw3 =
                ...
                if (rw2 != null && rw3 != null) {
                    ...
                    if (rw2write && rw3write) {
                        onSerialConnectError(
                                new IOException(
                                        "multiple write characteristics ("
                                                + rw2prop
                                                + "/"
                                                + rw3prop
                                                + ")"));
                        ...
                        onSerialConnectError(
                                new IOException(
                                        "no write characteristic (" + rw2prop + "/" + rw3prop));
                        return;
                    }
                }
            }
        ...
        if ((readProperties & BluetoothGattCharacteristic.PROPERTY_INDICATE) != 0) {
            ...
            onSerialConnectError(
                    new IOException(
                            "no indication/notification for read characteristic ("
                                    + readProperties
                                    + ")"));
            return;
        }
