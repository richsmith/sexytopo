...
public class TherionImporterTest {
    ...
    private static final List<String> APPENDED_LINES =
            Arrays.asList(APPENDED_STYLE_TEXT.split("\n"));
    ...
    private static final String INLINE_STYLE_TEXT =
            "encoding utf-8\n"
                    ...
                    + "data normal from to length compass clino\n"
                    + "1\t-\t1.677\t271.10\t6.59\n"
                    + "2\t1\t5.541\t73.93\t-4.67\t\t#  {from: 5.542 73.95 -4.64, 5.541 73.93 -4.69,"
                    + " 5.541 73.92 -4.67} stn 1 is drilled hole\n"
                    + "2\t-\t1.371\t140.88\t76.92\n"
                    + "3\t2\t2.151\t62.39\t27.97\t\t#  {from: 2.151 62.41 27.97, 2.152 62.36 27.97,"
                    + " 2.151 62.39 27.99} drilled hole\n"
                    + "3\t-\t1.846\t116.92\t85.17\n"
                    + "\n"
                    + "extend start 1\n"
                    + "endcentreline\n"
    ...
    public void testInlineStyleComments() throws Exception {
        ...
        // Backward leg "2  1  ... # ... stn 1 is drilled hole" puts comment on station 2 (the new
        // one)
        Station station2 = survey.getStationByName("2");
        Assert.assertNotNull(station2.getComment());
        Assert.assertTrue(station2.getComment().contains("stn 1 is drilled hole"));
    ...
    public void testBothStylesProduceSameStationCount() throws Exception {
        ...
        Assert.assertEquals(
                appendedSurvey.getAllStations().size(), inlineSurvey.getAllStations().size());
    }
